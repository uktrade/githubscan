...
class Command(BaseCommand):
    ...
    def email_org_report(self):
        try:
            ...
            emails = [obj.email for obj in OrganisationNotificationTarget.objects.all()]
            self.__send_email__(emails, data, settings.NOTIFY_TEMPLATE_ID)
            self.stdout.write(
                self.style.SUCCESS("Org Email Sent to: {}".format(",".join(emails)))
            )
    ...
    def email_team_reports(self):
        ...
        for team in Team.objects.filter(reporting_enabled=True):
            try:
                notification_targets = TeamNotificationTarget.objects.filter(team=team)
        ...
        for team in Team.objects.filter(reporting_enabled=True):
            try:
                ...
                if data["content"] and emails:
                    self.__send_email__(
                        emails, data, settings.NOTIFY_DETAILED_VULNURABILITY_TEMPLATE_ID
                    )
                    self.stdout.write(
                        self.style.SUCCESS(
