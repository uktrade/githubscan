...
class Updater:
    ...
    def __repositories__(self):
        ...
        dbRepositoriesSet = set(repositoriesInDb.values_list("name", flat=True))
    ...
    def __set_skip_scan__(self):
        ...
        """
        repositoriesInDb = set(self.db_client.getRepos().values_list("name", flat=True))
        for repository in repositoriesInDb:
            ...
            if topics:
                if self.skip_topic in topics:
                    Repository.objects.filter(name=repository).update(skip_scan=True)
    ...
    def __teamRepositories__(self):
        ...
        for team in teamsInDb:
            teamRepositoriesInGitHub = set(self.github_client.getTeamRepos(team=team))

            teamRepositoriesInDB = set(
            ...
            if teamRepositoriesInDB != teamRepositoriesInGitHub:
                Team(name=team).repositories.set(teamRepositoriesInGitHub)
    ...
    def __vulnerabilities__(self):
        ...
        repositories = set(self.db_client.getRepos().values_list("name", flat=True))
        ...
        for repository in repositories:
            ...
            alertsInDb = (
                self.db_client.getDetailsRepoVulnerabilities(repository=repository)
            ).annotate(
                publishedAt_as_string=Func(
                    F("published_at"),
            ...
            if active_repo_alerts:
                ...
                alertsSetInGithub = set(tuple(row) for row in active_repo_alerts)
                ...
                for record in remove_records:
                    alertsInDb.filter(
                        ...
                        severity_level=record[2],
                        identifier_type=record[3],
                        identifier_value=record[4],
                        advisory_url=record[5],
                    ).delete()

                for record in add_records:
                    RepositoryVulnerability(
                        repository=self.db_client.getRepo(repository=record[0]),
                        package_name=record[1],
                        severity_level=record[2],
                        identifier_type=record[3],
                        identifier_value=record[4],
    ...
    def __update_vulnerability_age__(self):
        ...
        for record in RepositoryVulnerability.objects.all():
            ...
            RepositoryVulnerability.objects.filter(id=record.id).update(
                publish_age_in_days=publish_age_in_days,
                detection_age_in_days=detection_age_in_days,
            )
        ...
        for vulnerablity in RepositoryVulnerability.objects.all():
            ...
            if vulnerablity.detection_age_in_days >= max_critical_alert_age:
                ...
                if vulnerablity.severity_level == "critical":
                    ...
                    time_since_current_level = (
                        vulnerablity.detection_age_in_days - max_critical_alert_age
                    )

                if vulnerablity.severity_level == "high":
                    if (
                        vulnerablity.detection_age_in_days >= max_high_alert_age
                        and vulnerablity.detection_age_in_days
                        < max_high_alert_age + max_critical_alert_age
                    ):
                    ...
                    if (
                        vulnerablity.detection_age_in_days
                        >= max_high_alert_age + max_critical_alert_age
                    ):
                        effective_severity_level = "SLA_BREACH"
                        time_since_current_level = (
                ...
                if vulnerablity.severity_level == "moderate":
                    if (
                        vulnerablity.detection_age_in_days >= max_moderate_alert_age
                        and vulnerablity.detection_age_in_days
                        < max_moderate_alert_age + max_high_alert_age
                    ):
                    ...
                    if (
                        ...
                        >= max_moderate_alert_age + max_high_alert_age
                        and vulnerablity.detection_age_in_days
                        < max_moderate_alert_age
                        + max_high_alert_age
                        + max_critical_alert_age
                    ):
                        effective_severity_level = "critical"
                        time_since_current_level = (
                            vulnerablity.detection_age_in_days
            ...
            RepositoryVulnerability.objects.filter(id=vulnerablity.id).update(
                ...
                time_since_current_level=time_since_current_level,
            )
    ...
    def __update_slo_breach_status__(self):
        ...
        for critical_alert_record in RepositoryVulnerability.objects.filter(
            severity_level="critical"
        ).all():
            if critical_alert_record.detection_age_in_days > max_critical_alert_age:
                RepositoryVulnerability.objects.filter(
                    id=critical_alert_record.id
                ).update(slo_breach=True)
            else:
                RepositoryVulnerability.objects.filter(
                    id=critical_alert_record.id
                ).update(slo_breach=False)

        for high_alert_record in RepositoryVulnerability.objects.filter(
            severity_level="high"
        ).all():
            if high_alert_record.detection_age_in_days > max_high_alert_age:
                RepositoryVulnerability.objects.filter(id=high_alert_record.id).update(
                    slo_breach=True
                )
            else:
                RepositoryVulnerability.objects.filter(id=high_alert_record.id).update(
        ...
        for moderate_alert_record in RepositoryVulnerability.objects.filter(
            severity_level="moderate"
        ).all():
            if moderate_alert_record.detection_age_in_days > max_moderate_alert_age:
                RepositoryVulnerability.objects.filter(
                    id=moderate_alert_record.id
                ).update(slo_breach=True)
            else:
                RepositoryVulnerability.objects.filter(
                    id=moderate_alert_record.id
    ...
    def __update_counts__(self):
        ...
        repositories = self.db_client.getVulnerableRepositories().values_list(
        ...
        for repository in repositories:
            ...
            critical_count = RepositoryVulnerability.objects.filter(
                repository=repository_obj, severity_level="critical"
            ).count()
            high_count = RepositoryVulnerability.objects.filter(
                repository=repository_obj, severity_level="high"
            ).count()
            moderate_count = RepositoryVulnerability.objects.filter(
                repository=repository_obj, severity_level="moderate"
            ).count()
            low_count = RepositoryVulnerability.objects.filter(
                repository=repository_obj, severity_level="low"
            ).count()

            effective_slabreach_count = RepositoryVulnerability.objects.filter(
                repository=repository_obj, effective_severity_level="SLA_BREACH"
            ).count()

            effective_critical_count = (
                critical_count
                + RepositoryVulnerability.objects.filter(
                    repository=repository_obj, effective_severity_level="critical"
                ).count()
            ) - RepositoryVulnerability.objects.filter(
            ...
            effective_high_count = (
                high_count
                + RepositoryVulnerability.objects.filter(
                    repository=repository_obj, effective_severity_level="high"
                ).count()
            ) - (
                RepositoryVulnerability.objects.filter(
                    repository=repository_obj,
            ...
            effective_moderate_count = (
                moderate_count
                + RepositoryVulnerability.objects.filter(
                    repository=repository_obj, effective_severity_level="moderate"
                ).count()
            ) - (
                RepositoryVulnerability.objects.filter(
                    repository=repository_obj,
            ...
            effective_low_count = low_count - (
                ...
                + RepositoryVulnerability.objects.filter(
                    repository=repository_obj,
                    severity_level="low",
                    effective_severity_level="moderate",
                ).count()
            )

            RepositoryVulnerabilityCount(
                repository=repository_obj,
                critical=critical_count,
                high=high_count,
                moderate=moderate_count,
                low=low_count,
                effective_slabreach=effective_slabreach_count,
                effective_critical=effective_critical_count,
                effective_high=effective_high_count,
                effective_moderate=effective_moderate_count,
                effective_low=effective_low_count,
            ).save()
    ...
    def __update_slo_breach_count__(self):
        ...
        if breaches:
            repositories = breaches.values_list("repository", flat=True).distinct()
    ...
    def __updat_team_vulnerability_count(self):
        ...
        for team in teams:
            team_repositories = set(
                self.db_client.getTeamRepos(team=team.name)
                .repositories.all()
