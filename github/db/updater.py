...
class Updater:
        ...
        for repository in repositories:
            ...
            if alertsInGithub:
                for alert in alertsInGithub:
                    ...
                    active_repo_alerts.append([repository,package_name,severity_level,identifier_type,identifier_value,advisory_url,published_at])

            #add/remove alerts
            alertsInDb = self.db_client.getDetailsRepoVulnerabilities(repository=repository)

            # if repo has active alert check if therre are new to add or have old to delete
            if active_repo_alerts:
                alertsSetInDb = set(alertsInDb.values_list('repository','package_name','severity_level','identifier_type','identifier_value','advisory_url','published_at'))
                ...
                for record in remove_records:
                    alertsInDb.filter(repository=record[0],package_name=record[1],severity_level=record[2],identifier_type=record[3],identifier_value=record[4],advisory_url=record[5]).delete()

                for record in add_records:
                    RepositoryVulnerability(repository=self.db_client.getRepo(repository=record[0]),package_name=record[1],severity_level=record[2],identifier_type=record[3],identifier_value=record[4],advisory_url=record[5],published_at=record[6]).save()
            else:
    ...
    def __update_vulnerability_age__(self):
        ...
        for record in RepositoryVulnerability.objects.all():
            ...
            RepositoryVulnerability.objects.filter(id=record.id).update(publish_age_in_days=publish_age_in_days,detection_age_in_days=detection_age_in_days)
    ...
    def __update_slo_breach_status__(self):
        ...
        for critical_alert_record in RepositoryVulnerability.objects.filter(severity_level='critical').all():
            if critical_alert_record.publish_age_in_days > max_critical_alert_age:
                RepositoryVulnerability.objects.filter(id=critical_alert_record.id).update(slo_breach=True)
        ...
        for high_alert_record in RepositoryVulnerability.objects.filter(severity_level='high').all():
            if high_alert_record.publish_age_in_days > max_high_alert_age:
                RepositoryVulnerability.objects.filter(id=high_alert_record.id).update(slo_breach=True)
        ...
        for moderate_alert_record in RepositoryVulnerability.objects.filter(severity_level='moderate').all():
            if high_alert_record.publish_age_in_days > max_moderate_alert_age:
                RepositoryVulnerability.objects.filter(id=moderate_alert_record.id).update(slo_breach=True)
    ...
    def __update_slo_breach_count__(self):
        ...
        if breaches:
            ...
            for repository in repositories:
                repository_obj = self.db_client.getRepo(repository=repository)
                critical_count = breaches.filter(repository=repository_obj, severity_level='critical').count()
                high_count = breaches.filter(repository=repository_obj, severity_level='high').count()
                moderate_count = breaches.filter(repository=repository_obj, severity_level='moderate').count()
                low_count = breaches.filter(repository=repository_obj, severity_level='low').count()

                RepositorySLOBreachCount(repository=repository_obj,critical=critical_count,high=high_count,moderate=moderate_count,low=low_count).save()
