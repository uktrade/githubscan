...
class Updater:
        ...
        for repository in repositoriesInDb:
            ...
            if topics:
                if self.skip_topic in topics:
                    GitHubRepo.objects.filter(name=repository).update(skip_scan=True)
    ...
    def __teamRepositories__(self):
        ...
        for team in teamsInDb:
            teamRepositoriesInGitHub = self.github_client.getTeamRepos(team=team)
            ...
            for repository in add_records:
                GitHubTeamRepo(team=self.db_client.getTeam(team=team), repository=self.db_client.getRepo(repository=repository)).save()         
    ...
    def __vulnerabilities__(self):
        ...
        for repository in repositories:
            alertsInGithub = self.github_client.getVulnerabilityAlerts(repository=repository)
            if alertsInGithub:
                for alert in alertsInGithub:
                    GitHubRepoVulnerabilites(repository=self.db_client.getRepo(repository=repository),package_name=alert[0],severity_level=alert[1],identifier_type=alert[2],identifier_value=alert[3],advisory_url=alert[4]).save()


    def __update_counts_(self):
        #Drop all before updating
        GitHubVulnerabilityCount.objects.all().delete()
        repositories = self.db_client.getVulnerableRepositories().values_list('repository',flat=True)

        for repository in repositories:
            repository_obj = self.db_client.getRepo(repository=repository)
            critical_count = GitHubRepoVulnerabilites.objects.filter(repository=repository_obj,severity_level='critical').count()
            high_count = GitHubRepoVulnerabilites.objects.filter(repository=repository_obj,severity_level='high').count()
            moderate_count = GitHubRepoVulnerabilites.objects.filter(repository=repository_obj,severity_level='moderate').count()
            low_count = GitHubRepoVulnerabilites.objects.filter(repository=repository_obj,severity_level='low').count()
            GitHubVulnerabilityCount(repository=repository_obj,critical=critical_count,high=high_count,moderate=moderate_count,low=low_count).save()
