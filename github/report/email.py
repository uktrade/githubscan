...
class EmailReport(Report):
    ...
    def getReport(self):
        ...
        data.update(
            self.__format_email_report__(repositories_of_interest=self.getReportRepos())
        )
        data.update(
    ...
    def getTeamReport(self, team):
        ...
        data = {"vulnerable_repo_count": len(vulnerable_repos)}
        data.update(
            self.__format_email_report__(
    ...
    def getDetailedTeamReport(self, team):
        ...
        data = {"vulnerable_repo_count": len(vulnerable_repos)}
        data.update(
            self.__detailed_repository_report(
                repositories_of_interest=vulnerable_repos,
                team=team,
            )
        )
    ...
    def __format_email_report__(self, repositories_of_interest, teams=None):
        ...
        for vulnerable_repository in report_repositories:
            ...
            if slo_breache_counts:
                ...
                content += "## SLO breach ##\n * Critical breach: {}\n * High breach: {}\n * Moderate breach: {}\n\n".format(
                    slo_breache_counts.critical,
                    slo_breache_counts.high,
                    slo_breache_counts.moderate,
                )
            ...
            for vulnerability in repository_vulnerabilities:
                csv_data.append(
                    [
                        ...
                        vulnerability.identifier_type,
                        vulnerability.identifier_value,
                        vulnerability.advisory_url,
                        github_alerts_link,
                        vulnerability.slo_breach,
                        vulnerability.publish_age_in_days,
                        vulnerability.detection_age_in_days,
                    ]
                )
        ...
        if report_repositories.aggregate(sum=Sum("effective_slabreach"))["sum"]:
            ...
            if (
                report_repositories.aggregate(sum=Sum("effective_critical"))["sum"]
                != report_repositories.aggregate(sum=Sum("critical"))["sum"]
            ):
                subject_prefix = "AMBER"
            if (
                report_repositories.aggregate(sum=Sum("effective_high"))["sum"]
                != report_repositories.aggregate(sum=Sum("high"))["sum"]
            ):
                subject_prefix = "AMBER"
            if (
                report_repositories.aggregate(sum=Sum("effective_moderate"))["sum"]
                != report_repositories.aggregate(sum=Sum("moderate"))["sum"]
            ):
                subject_prefix = "AMBER"
        ...
        # SLO Breach Summary
        slo_breached_repositories_of_interest = set(
            self.db_client.getSloBreachRepos().values_list("repository", flat=True)
        ).intersection(set(report_repositories.values_list("repository", flat=True)))
        ...
        summary += "#SLO Breach Summary\n * total Repositories: {}\n * total Critical breach: {}\n * total High breach: {}\n * total Moderate breach: {}\n\n".format(
            ...
            slo_breached_report_repositories.aggregate(sum=Sum("critical"))["sum"],
            slo_breached_report_repositories.aggregate(sum=Sum("high"))["sum"],
    ...
    def __detailed_repository_report(self, repositories_of_interest, team):
        ...
        vulnerabilities = self.db_client.getRepositoriesVulnerabilities(
            repositories=repositories_of_interest
        )
        ...
        sorted_uniq_vulnerabilities = (
            ...
            .annotate(
                ranking=Case(
                    ...
                    When(severity_level="moderate", then=3),
                    When(severity_level="low", then=4),
                ),
                effective_ranking=Case(
                    When(effective_severity_level="SLA_BREACH", then=1),
                    When(effective_severity_level="critical", then=2),
                    When(effective_severity_level="high", then=3),
                    When(effective_severity_level="moderate", then=4),
                    When(effective_severity_level="low", then=5),
                ),
            )
        ...
        for index, v in enumerate(sorted_uniq_vulnerabilities):
            ...
            current_severity_level = (
                v["effective_severity_level"] or v["severity_level"]
            )
            ...
            days_left_to_breach_key = (
                f"Time Till {(current_severity_level).capitalize()} Breach"
            )
            ...
            if current_severity_level == "SLA_BREACH":
                ...
                days_left_to_breach_key = (
                    f"Time Till {(current_severity_level).capitalize()}"
                )
                days_left_to_breach = (
                    f"{v.time_since_current_level} day(s) over SLA Budget"
                )
            ...
            if index == 0:
                ...
                subject_prefix = (
                    f"Action by: {due_day}{due_day_suffix} {due_month} {due_date.year}"
                )
                if current_severity_level == "SLA_BREACH":
        ...
        return {
            "content": content,
            "csv": "",
            "subject_prefix": f"[{subject_prefix}] {vulnerable_package_count} Vulnerable packages",
            "summary": "",
        }
