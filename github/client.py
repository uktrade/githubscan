...
class GHClient:
    ...
    def __GithubResponse(self, payload):
        return self.session.post(self.GITHUB_API_URL, data=payload)
    ...
    def getRepos(self):
        ...
        repos = list()
        query = self.__openQuery(
            os.path.join(self.APP_ROOT, "github", "gqlQueries", "repos.gql")
        )
        ...
        while True:
            ...
            if (
                response["data"]["organization"]["repositories"]["pageInfo"][
                    "hasNextPage"
                ]
                is False
            ):
                ...
                query_variables.update(
                    {
                        "after": response["data"]["organization"]["repositories"][
                            "pageInfo"
                        ]["endCursor"]
                    }
                )
    ...
    def getRepoTopics(self, repository):
        ...
        topics = list()
        query = self.__openQuery(
            os.path.join(self.APP_ROOT, "github", "gqlQueries", "repoTopics.gql")
        )
        query_variables = {
        ...
        while True:
            ...
            edges = response["data"]["organization"]["repository"]["repositoryTopics"][
                "edges"
            ]
            ...
            if (
                response["data"]["organization"]["repository"]["repositoryTopics"][
                    "pageInfo"
                ]["hasNextPage"]
                is False
            ):
                ...
                query_variables.update(
                    {
                        "after": response["data"]["organization"]["repository"][
                            "repositoryTopics"
                        ]["pageInfo"]["endCursor"]
                    }
                )
    ...
    def getTeams(self):
        ...
        teams = list()
        query = self.__openQuery(
            os.path.join(self.APP_ROOT, "github", "gqlQueries", "teams.gql")
        )
        query_variables = {"org_name": self.ORG_NAME, "first": self.first}
        ...
        while True:
            ...
            if (
                response["data"]["organization"]["teams"]["pageInfo"]["hasNextPage"]
                is False
            ):
                break
            else:
                query_variables.update(
                    {
                        "after": response["data"]["organization"]["teams"]["pageInfo"][
                            "endCursor"
                        ]
                    }
                )
    ...
    def getTeamRepos(self, team):
        ...
        query = self.__openQuery(
            os.path.join(self.APP_ROOT, "github", "gqlQueries", "teamRepo.gql")
        )
        query_variables = {"org_name": self.ORG_NAME, "team": team, "first": self.first}
        ...
        while True:
            for edge in response["data"]["organization"]["team"]["repositories"][
                "edges"
            ]:
                if not edge["node"]["isArchived"] and (
                    edge["permission"] == "WRITE"
                    or edge["permission"] == "ADMIN"
                    or edge["permission"] == "MAINTAIN"
                ):
                    teamrepos.append(edge["node"]["name"])
            if (
                response["data"]["organization"]["team"]["repositories"]["pageInfo"][
                    "hasNextPage"
                ]
                is False
            ):
                ...
                query_variables.update(
                    {
                        "after": response["data"]["organization"]["team"][
                            "repositories"
                        ]["pageInfo"]["endCursor"]
                    }
                )
    ...
    def getVulnerabilityAlerts(self, repository):
        ...
        while True:
            nodes = response["data"]["organization"]["repository"][
                "vulnerabilityAlerts"
            ]["nodes"]
            ...
            if nodes is not None:
                for node in nodes:
                    if node["state"] == "OPEN":
                        ...
                        if (
                            node["securityVulnerability"]["firstPatchedVersion"]
                            is not None
                        ):
                            patched_version = node["securityVulnerability"][
                                "firstPatchedVersion"
                            ]["identifier"]
                ...
                if not response["data"]["organization"]["repository"][
                    "vulnerabilityAlerts"
